...
//      replyDAO.selectAll().stream().filter(dto -> dto.getReplyGroup() == 6).map(ReplyDTO::toString).forEach(System.out::println);

//      대댓글 숨기고 댓글만 전체 조회
//      replyDAO.selectAll().stream().filter(dto -> dto.getReplyCount() != 0).map(ReplyDTO::toString).forEach(System.out::println);

//      대댓글 삭제
//      replyDAO.deleteReReply(8L);

//      댓글 삭제(대댓글까지 삭제)
//      replyDAO.deleteReply(6L);

//      댓글 수정
//      replyDAO.selectAll().stream().filter(dto -> dto.getReplyId() == 2).map(ReplyDTO::toReplyVO).forEach(vo -> {
