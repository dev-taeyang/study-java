package arrayTest; // for 문 if 문사용
...
//      버튼 배열
      String[] arButton = {"btn1", "btn2", "btn3", "btn4"}; // 4개 짜리 배열
//      성공 메세지 배열
...
//         만약 temp에 0이 담겨있다면, 0을 유지해야 한다.
         temp = temp == 0 ? 0 : choice; // temp 가 0 일때 true -> 0 false -> choice
//         사용자가 선택한 버튼 번호
         choice = sc.nextInt(); // 1받을때
...
//         전 단계를 진행 했는 지 검사
         condition = choice - 1 == temp; // temp가 처음에 0으로 있는데 choice가 바로 2가되면  condition이 false이므로
